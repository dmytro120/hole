class ACContextMenu extends ACControl
{
	...
	static open(e)
	{
		...
		let target = e.target;
		if (!target.contextMenu) target = target.parentNode; // if clicked in td, use context menu of tr
		if (!target.contextMenu) return; // things inside td (e.g. input box) do not trigger context menu
		
		new ACContextMenu(null, e, target);
		e.preventDefault();
	}
}
ACContextMenu.lastContextMenu = null;
